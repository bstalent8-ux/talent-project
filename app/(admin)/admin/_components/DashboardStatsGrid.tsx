...
const CATEGORY_STYLE: Record<string, { color: string; icon: React.ReactNode }> = {
  ugc:   { color: "#C98A70", icon: <Video size={20} /> },
  model: { color: "#B9694C", icon: <Sparkles size={20} /> },
};
const FALLBACK_CATEGORY_COLORS = ["#087F83", "#C98A70", "#1EA672", "#D98B7B", "#C98A70"];
...
export default function DashboardStatsGrid({ stats }: { stats: AdminDashboardStats }) {
  ...
  return (
    ...
    >
      <DashboardCard label={t.newRegistrations} value={stats.newRegistrations} color="#087F83" icon={<UserPlus size={20} />} />
      ...
      <DashboardCard label={t.reviews}   value={stats.reviews}   color="#D98B7B" icon={<Star size={20} />} />

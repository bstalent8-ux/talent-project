...
export default function BrandsTable({ brands, total, page, pageSize, status, sort, dir }: Props) {
  ...
  const confirmCfg = modal ? {
    ...
    reset:   { color: "#E7A58A", msg: t.confirmReset,   label: t.reset   },
    block:   { color: "#EF4444", msg: t.confirmBlock,   label: t.block   },
    unblock: { color: "var(--color-primary-text)", msg: t.confirmUnblock, label: t.unblock },
  }[modal.type] : null;
                    ...
                    <tr key={b.id}>
                      ...
                      <td style={cellStyle}>
                        <span style={{
                          ...
                          backgroundColor: isBlocked ? "rgba(239,68,68,0.15)" : "rgba(8,127,131,0.12)",
                          color: isBlocked ? "#EF4444" : "#087F83",
                        }}>
                      ...
                      <td style={cellStyle}>
                        {b.taxDocumentUrl ? (
                          <a href={b.taxDocumentUrl} target="_blank" rel="noopener noreferrer"
                            style={{ color: "#4FA7A3", display: "flex", alignItems: "center", gap: 4, fontSize: 12 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: showReason ? 14 : 0 }}>
              ...
              {modal.brand.taxDocumentUrl && (
                <div>
                  ...
                  <a href={modal.brand.taxDocumentUrl} target="_blank" rel="noopener noreferrer" style={{ color: "#4FA7A3", display: "flex", alignItems: "center", gap: 4, fontSize: 12.5 }}>
                    <ExternalLink size={13} /> {t.view}
                  </a>
                </div>
              )}

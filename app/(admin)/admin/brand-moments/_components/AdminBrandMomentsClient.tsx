...
const TX = {
  ar: {
    title: "لحظات البراندات — الصفحة الرئيسية", image: "الصورة", subject: "العنوان", status: "الحالة",
    ...
    confirmApprove: "الموافقة على هذه الصورة وإظهارها في الصفحة الرئيسية؟",
    confirmReject: "رفض هذه الصورة؟",
    reasonLabel: "سبب الرفض (اختياري)",
    noRequests: "لا توجد لحظات بعد",
  },
  en: {
    title: "Brand Moments — Home Page", image: "Image", subject: "Title", status: "Status",
    submitted: "Submitted", actions: "Actions",
    all: "All", pending: "Pending", approved: "Approved", rejected: "Rejected",
    approve: "Approve", reject: "Reject",
    confirmApprove: "Approve this photo and show it on the home page?",
    confirmReject: "Reject this photo?",
    reasonLabel: "Rejection reason (optional)",
    noRequests: "No brand moments yet",
  },
};

type ModalState = { type: "approve" | "reject"; id: string };

export default function AdminBrandMomentsClient({ moments }: { moments: AdminBrandMoment[] }) {
  ...
  async function runModal() {
    if (!modal) return;
    setLoading(true);
    await fetch(`/api/admin/landing/brand-moments/${modal.id}`, {
      method: "PATCH",
              ...
              <tbody>
                {filtered.map((v) => {
                  ...
                  return (
                    <tr key={v.id}>
                      <td style={cellStyle}>
                        <div style={{ width: 56, height: 40, borderRadius: 8, overflow: "hidden", backgroundColor: BORDER, position: "relative" }}>
                          <Image src={v.imageUrl} alt="" fill style={{ objectFit: "cover" }} />
                        </div>
                      </td>
                      <td style={cellStyle}>
                        <div style={{ fontWeight: 600 }}>{v.title}</div>
                        <div style={{ color: MUTED, fontSize: 11 }}>
                          {v.location || (v.submitterHandle ? `@${v.submitterHandle}` : "—")}
                        </div>
                      </td>
                      <td style={cellStyle}>

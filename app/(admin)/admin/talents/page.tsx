...
export default async function AdminTalentsPage({ searchParams }: Props) {
  ...
  return (
    <AdminTalentsShell status={status} category={category} city={city} duplicate={duplicate} filterOptions={filterOptions}>
      <Suspense key={`${page}-${status}-${pageSize}-${category}-${city}-${sort}-${dir}-${duplicate}`} fallback={<TalentsTableSkeleton />}>
        <TalentsTableSection page={page} pageSize={pageSize} status={status} category={category} city={city} sort={sort} dir={dir} duplicate={duplicate} />
      </Suspense>
    </AdminTalentsShell>
  );
}

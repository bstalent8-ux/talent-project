...
import { STAGE_FIELD_TYPES, type CandidateStage, type StageFieldType } from "@/features/candidates/types";
...
const TX = {
  ar: {
    ...
    candidatesOnStage: (n: number) => `${n} مرشح عليها`,
    delete: "مسح", deleteTitle: "مسح المرحلة؟",
    deleteDesc: (n: number) => n > 0 ? `${n} مرشح هيتنقلوا للمرحلة التانية القريبة تلقائي.` : "مفيش مرشحين عليها دلوقتي.",
    cancel: "إلغاء",
  ...
  en: {
    ...
    candidatesOnStage: (n: number) => `${n} candidate(s) on it`,
    delete: "Delete", deleteTitle: "Delete this stage?",
    deleteDesc: (n: number) => n > 0 ? `${n} candidate(s) will move to the next nearest stage automatically.` : "No candidates on it right now.",
    cancel: "Cancel",
...
// Recruitment-pipeline stages sub-panel of CandidateSettingsPanel — mirrors
// leads' StagesTab exactly, targeting /api/admin/candidates/stages.
export default function CandidateStagesTab({ stages, onChanged }: { stages: CandidateStage[]; onChanged: () => void }) {
  ...
  const [deleteTarget, setDeleteTarget] = useState<CandidateStage | null>(null);
  ...
  async function createStage(e: React.FormEvent) {
    ...
    const res = await fetch("/api/admin/candidates/stages", {
  ...
  async function moveStage(id: string, direction: "up" | "down") {
    setBusy(true);
    await fetch(`/api/admin/candidates/stages/${id}/move`, {
  ...
  async function confirmDeleteStage() {
    ...
    setBusy(true);
    await fetch(`/api/admin/candidates/stages/${deleteTarget.id}`, { method: "DELETE" }).catch(() => {});
    setBusy(false);
  ...
  async function addField(stageId: string, e: React.FormEvent) {
    ...
    const options = newField.fieldType === "select"
      ...
      : undefined;
    await fetch(`/api/admin/candidates/stages/${stageId}/fields`, {
      method: "POST", headers: { "Content-Type": "application/json" },
  ...
  async function removeField(fieldId: string) {
    setBusy(true);
    await fetch(`/api/admin/candidates/stages/fields/${fieldId}`, { method: "DELETE" }).catch(() => {});
    setBusy(false);
    onChanged();
  }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <div key={stage.id} style={{ border: `1px solid ${BORDER}`, borderRadius: 10, overflow: "hidden" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 12px" }}>
              ...
              >
                ...
                <span style={{ fontSize: 11, color: MUTED }}>· {t.candidatesOnStage(stage.candidateCount)}</span>
              </button>
      ...
      <ConfirmationModal
        open={!!deleteTarget}
        title={t.deleteTitle}
        description={deleteTarget ? (stages.length <= 1 ? t.cantDeleteLast : t.deleteDesc(deleteTarget.candidateCount)) : undefined}

...
**Admin sidebar groups animate (2026-09-26):** a group's items are always mounted inside `.admin-group-body` (grid rows `0fr` ↔ `1fr`, 0.3 s) with `inert` while closed, and each `.admin-group-item` fades/slides in with a 30 ms stagger (`--i`); the chevron turns on the same curve. The CSS lives in `AdminSidebar`'s style block; `prefers-reduced-motion` turns it off. No animation plays on page load — the active group simply starts open.
...
**Bookings pages on the palette (2026-09-26):** `/bookings` (`BookingsClient.module.css` `.dark`/`.light` vars → chocolate/cream, `--green`/`--gold` → `--color-primary-text` / `--color-accent-strong`) and every `/bookings/[id]` component (`BookingDetail`, `BookingTimeline`, `BriefView`, `DeliverablesForm`, `PackageRequestView`, `ReviewForm`). Filled action buttons are `--color-primary` + `--color-primary-ink` (peach ones `--color-accent` + `--color-on-accent`); the "accepted" status chip is Soft Teal instead of violet. Status reds are unchanged.

**Remaining user pages on the palette (2026-09-26):** `/settings` (all sections), `/about`, `/chat` (full page), `/become-talent`, `/dashboard`, `/contact` + `components/contact/ContactForm`, `components/legal/LegalLayout` (terms/privacy/cookies), `/jobs/[id]` + applications, `/profile/me` + `/profile/me/complete` leftovers, `/community/question/[id]`, `/notifications`, `/blog` + `BlogCard` + `/blog/[slug]`, `components/packages/PackagePricing.module.css`, and the leftover dark navy tones in the `/jobs`, `/community`, `/favorites` CSS modules. Blog's purple accent became peach (`--color-accent-strong`); blog category colours and `BrandsCard` placeholder colours moved to teal/peach/brown tints. `/become-talent`'s gradient headline text is now solid teal and its CTA gradients are flat `--color-primary`. Still on the old colours: the 4 email templates in `lib/email/`, the unused old landing (`home/_components/landing/`, `components/home/TrustedBrands`), favicon/OG image.

**Explore final CTA (2026-09-26):** "Ready to launch your next campaign?" restyled to the reference — chocolate→dark-teal band, peach orb + arc (start side) and teal orb + arc + shard (end side) as inline SVG in `ExploreClient.tsx` (`.ctaOrbs`, mirrored in RTL), copy and buttons in a frosted glass panel, peach primary with glow and a dark outlined ghost. Still guests only (`myRole === null`). Its top/bottom edges dissolve into the page (a registered `--cta-fade` length drives a vertical mask on `::before` and the orb SVGs, not on the section, so its shadow isn't clipped); on hover the fade animates to 0 and the band gets a drop shadow.

**Explore "Why Talents" band (2026-09-26):** restyled to the reference: kicker + serif heading on the reading-start side with a peach hand-drawn underline (`.bandSwoosh`, inline SVG data URI), four cards in one row (2 under 1100 px, 1 under 560 px) with peach icon tiles, on a peach wash with two decorative inline SVGs in `ExploreClient.tsx` — fine line-art (`.bandLines`) and leaves (`.bandLeaves`, mirrored in RTL). Copy unchanged.

**Explore cover (2026-09-26):** the hero background is now the brand cover art `public/assets/explore-cover-{768,1280,1920,2172}.webp` (from a 2172×724 PNG, 25–100 KB) via `srcSet`, replacing the Unsplash photo; `.heroOverlay` was lightened to a centre vignette + bottom fade so the creators at the sides stay visible.

**Explore load speed (2026-09-26):** Lighthouse mobile on production `/explore` was 59 (LCP 8.5 s). Server time was not the problem (warm TTFB ~0.35 s, cold starts ~1 s on every page alike); the LCP was the hero photo, a single 1600 px / 277 KB Unsplash image on phones. `ExploreHero.tsx` now ships a `srcSet` (480–1600 w, `sizes="100vw"`, `q=60`, `fetchPriority="high"`) — a 2× phone downloads the 768 w variant (~92 KB). The logo was a 800 px PNG served through `/_next/image` un-optimized (59 KB on every page); it is now `public/assets/talents-logo-{light,dark}.webp` (360 px, ~11–13 KB) rendered with `unoptimized`, in Navbar, Footer, AuthFrame, onboarding, AdminSidebar and `/profile/me`. The `.png` originals stay as source files. Meta Pixel (~190 KB) is still loaded `afterInteractive`: moving it to `lazyOnload` would drop the first PageView, because `trackMetaEvent` no-ops until the `fbq` stub exists.

**Auth pages on phones — app-like (2026-09-26):** under 640 px (`auth.module.css`, last two media blocks) the form is the screen, not a card: no border/background/shadow, a slim sticky top bar (safe-area aware), and the form centred vertically in the remaining height (Sign in sits mid-screen; Register scrolls). Inputs are 50 px tall with a 16 px font (iOS no longer zooms on focus), the submit button 52 px, top-bar buttons 40 px. Register's role pick is a two-up choice with the subtitles and icons hidden. **Register fits one phone screen with no scroll** (checked 375×812 and 375×667, EN + AR): 44 px inputs (42 on screens under 720 px tall), 8 px row gap, password + confirm side by side (`.fieldHalf`, lock icon hidden there), subheading hidden (`.authPage:has(.fieldGrid)`), shorter password placeholders. Validation errors add lines, so a form showing errors can still scroll. Sign in and Forgot password use the same phone sizes (44 px fields, 48 px button, xl heading, via `.authPage:not(:has(.fieldGrid))`) and fit a 320×568 screen; the Sign in email placeholder is now "Email or @username" / "الإيميل أو @username". Desktop and tablet are unchanged.

## Brand public page redesign — 2026-09-26

`/brand/[id]` (the page a talent sees) no longer renders `BrandHero` + `DynamicProfileRenderer`; it is a fixed layout under the normal Navbar: `app/(main)/brand/[id]/_components/BrandProfileShell.tsx` (hero: cover, logo, name/verified, kind, city, tags, Follow / Message / View Opportunities, website + social icons) and `BrandSections.tsx` (About + 4 facts, Active Opportunities, Recent Collaborations, Reviews; rail: Brand Stats, Verification & Trust, Talents They Worked With, Similar Brands), styled by `brandPage.module.css` (tokens only, one column under 1100 px). Any admin-configured *dynamic* brand sections are therefore not shown on the public page any more.

Data: the page route now calls `getBrandPageData()` (`features/brand-page/brand-page.service.ts`, inside the existing 10-min `cachedPublic` payload) — open `jobs` + `job_applications` counts, completed/paid `bookings` (collaborations, distinct talents, first 4 talents), verification (auth `email_confirmed_at`, `brand_profiles.status`, a `held`/`released` payment), `follows` count, similar approved brands by `category_id`/`industry`, and approved `brand_reviews`. Every read is independent and fails soft. Follow/unfollow uses the existing `/api/community/follow`; Message dispatches the chat widget's `open-chat-widget` event.

New migration `supabase/migrations/20260926_brand_public_page.sql` (hand-run, §6): `brand_profiles.cover_url / tagline / company_size / founded_year / tags` and a service-role-only `brand_reviews` table (talent → brand, one per booking, sub-ratings). Until it runs the page falls back (gradient cover with the brand name, "—" facts, industry/category as tags, preview reviews). **Not built yet:** any UI for a brand to set those new columns (or upload a cover), any route that writes `brand_reviews`, a response-rate computation, and a public collaboration-media source. Those blocks show sample content from `_components/preview.ts` under a visible "Preview" chip — remove each entry when its source is wired. `components/profile/brand/BrandHero.tsx` is now unused.

**Brand page on phones (2026-09-26):** under 760 px `brandPage.module.css` turns the page into one continuous surface: full-bleed cover, 116 px logo overlapping it, a Share button (native share sheet, clipboard fallback), the three actions in one row, sections separated by hairlines instead of cards. `.main`/`.rail` become `display: contents` and each section's `data-sec` sets its `order` (hero → stats → about → opportunities → gallery → talents → reviews → similar → trust). About clamps to 4 lines with Read more; facts sit in a tinted 2×2 box; opportunities, gallery, talents (round avatars + real `avg_rating`) and similar brands are horizontal swipe rows. A "Usually replies within N hours" row is preview-only (`PREVIEW_STATS.replyHours`). CSS-modules note: attribute selectors must carry a local class (`.card[data-sec=…]`) or the whole module fails to compile ("Selector is not pure").

## Brand onboarding + brand page wizard — 2026-09-26

- **Tour:** `/onboarding` was split into a shared `app/(auth)/onboarding/OnboardingTour.tsx` (`tx`, `finishHref` props). Talents keep `/onboarding` → `/profile/me/complete`; brands now get `/onboarding/brand` (5 steps: welcome, how hiring works, your brand page, verification & opportunities, tips) → `/profile/brand-setup`. `register/page.tsx` sends new brands to `/onboarding/brand` (was `/profile/me`).
- **Wizard** `/profile/brand-setup` (`app/(main)/profile/brand-setup/`, server page loads profile + `brand_profiles` + brand categories, client `BrandSetupWizard.tsx`): 1 Identity (logo via `/api/profile/avatar` + `AvatarCropModal`, cover via the new `/api/profile/brand-cover`), 2 Basics (name, tagline, category, industry, city), 3 About (bio, company size, founded year, tags ≤ 8), 4 Links (website + instagram/tiktok/youtube/facebook/linkedin/x), 5 Done (completion %, View page / Dashboard / Verify). Each step PATCHes only its own fields; a live preview of the page header sits beside the form on wide screens; phones get a sticky footer.
- **APIs:** `PATCH /api/profile/brand-setup` (brand only, zod `.strict()`; `profiles` full_name/city/bio, core via `profileService.updateCoreForUser` so the provider allowlist + status seeding still apply, `social_links` merged not replaced, extras direct → 409 `migration_required` if `20260926_brand_public_page.sql` isn't applied; the wizard only sends extras once `extrasReady`). `POST|DELETE /api/profile/brand-cover` (brand only, images ≤ 8 MB, Cloudinary `<folder>/brand-covers`, publishes without review like brand logos). Both call `invalidateBrand(handle)` + `invalidateBrand(userId)`.
- **Entry points:** own brand page shows "Edit page" (actions hidden while `GuestGuard` is loading so Follow doesn't flash); Settings → Company Details links to the wizard; `/profile/me` redirects brand accounts to the wizard; the chat bubble lifts above the wizard's sticky footer on phones.
- **Owner preview:** `/brand/[id]` now lets the brand itself see its own not-yet-approved page (owner match on id/handle is checked before anything is loaded); everyone else still gets 404 until `brand_profiles.status = approved`.

## Talent gender (Explore filter) — 2026-09-26

The Explore Male/Female filter compares `talent_profiles.social_links.gender`, but nothing in the app ever collected it, so all 54 approved talents had none and the filter emptied the grid. Gender is now stored in that key (no migration) as `male` / `female` only — `normalizeGender()` in `lib/profile-fields.ts` maps case / Arabic variants and returns `null` for anything else; public reads (`public-talents.service.ts`) go through it too.
- **Written by:** the complete-profile wizard (step 1, `/api/profile/complete`; **required** — "Save & continue" won't leave step 1 without Male/Female, and finishing the wizard with it empty jumps back there; "Save draft" still works), `/profile/me` (`POST /api/profile` normalizes it and drops the key on `null` / junk, so "not set" really clears it), and the admin talent editor ("النوع / Gender" select, for back-filling existing talents; `PATCH /api/admin/talents/[id]/profile` replaces `social_links` whole, and the editor rebuilds it from the advanced JSON + fields + gender, so nothing else is lost).
- **Not a completion field:** deliberately kept out of `TALENT_PHYSICAL_KEYS`, so setting only a gender does not complete the "physical" section (tested in `lib/profile-fields.test.ts`).
- **Explore:** the chips show counts ("Male (N)"), and "More filters" opens by itself while a gender filter is active. Until talents / admins fill it in, the counts stay low — that is data, not a bug.
- **Bulk back-fill page `/admin/talents/gender`** (linked from `/admin/talents` as "Set gender"): every talent (approved first) with photo, name, category and Male / Female / Clear buttons; each click saves at once via `PATCH /api/admin/talents/gender` (`requirePermission("talents","update")` + `getAdminUser()`, zod body `{ profileId, gender: male|female|null }`), which **merges** only the `gender` key into `social_links` and calls `invalidateTalent`. Tabs "No gender" / "All" with a set-count. Reads: `features/admin/services/talent-gender.service.ts`. Explore picks the change up within minutes (KV-backed `unstable_cache`, eventually consistent).
- **Gender chip counts follow the other filters (fix):** `ExploreClient`'s `passesBaseFilters()` is shared by the results and the Male/Female counts, so "Female (7)" always lists 7. The counts used to be over the whole unfiltered list (incl. non-UGC/Model talents Explore never shows), so they could exceed the results.

### Admin "approved" vs Explore count

`/admin/talents` "Approved" counts every `talent_profiles.status = approved`; Explore additionally needs a handle, `is_suspended = false`, `account_status` not blocked/suspended/rejected, and category UGC or Model. `lib/explore-visibility.ts` (`exploreHiddenReason()`, tested) encodes those rules and the admin talents table shows **"Hidden from Explore" + the reason** under the status badge of any approved talent that Explore drops. Keep it in step with `public-talents.service.ts` and `ExploreClient.tsx`.

**Explore gender filter (2026-09-26):** the Male/Female chips existed but matched nothing — gender was never collected (0 of 54 approved talents had it). Now stored in `talent_profiles.social_links.gender` (what `public-talents.service.ts` already read; no migration), always `male`/`female` via `normalizeGender()` in `lib/profile-fields.ts` (also accepts Arabic/any case). Written from: the talent wizard's Basic step (`CompleteProfileShell`, saved through `PATCH /api/profile/complete` section `physical`, which validates it), `/profile/me` (a select next to City), and the admin talent editor (own select; removed from the advanced-JSON box). `gender` is deliberately **not** in `TALENT_PHYSICAL_KEYS`, so it can't complete the physical section by itself (test: `lib/profile-fields.test.ts`). Explore shows per-gender counts on the chips and opens "More filters" while a gender filter is active. Existing talents stay unfiltered until an admin or the talent sets it.

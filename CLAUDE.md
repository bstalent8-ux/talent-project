...
- **Written by:** the complete-profile wizard (step 1, `/api/profile/complete`), `/profile/me` (`POST /api/profile` normalizes it and drops the key on `null` / junk, so "not set" really clears it), and the admin talent editor ("Gender (Explore filter)" select; `PATCH /api/admin/talents/[id]/profile` replaces `social_links` whole, and the editor rebuilds it from the advanced JSON + fields + gender, so nothing else is lost).
